import { useLocation } from "react-router-dom";
import GalataTower from "../media/images/galata.png";
import "tailwindcss/tailwind.css";

const AboutSection = () => {
    const location = useLocation();

    // Check if the page was navigated to directly
    const directNavigation = location.state?.directNavigation || false;

    return (
        <section
            id="about"
            className="flex flex-col-reverse lg:flex-row justify-between items-center py-10 gap-4"
        >
            <div className="flex flex-col items-start lg:w-1/2 mx-12 lg:pl-24 ">
                <h2 className="text-2xl lg:text-3xl font-bold italic text-white">
                    I am
                </h2>
                <ul className="list-disc pl-5 text-white font-['Roboto'] p-6 max-w-lg space-y-4 text-md md:text-lg">
                    <li>
                        a Stockholm-based front-end software developer and UX designer.
                    </li>
                    <li>
                        from Turkey’s Istanbul, the kingdom of cats, dogs, and seagulls!
                    </li>
                    <li>
                        a music enthusiast, gamer, party lover, and a friend of animals.
                    </li>
                    <li>
                        someone who enjoys designing and building digital architectures that are of help and
                        joy to others.
                    </li>
                </ul>
                <a 
                    href="/AboutPage"
                    className="secondary-button transition flex justify-center items-center gap-2 mt-4"
                    >
                    <span className="text-sm md:text-md font-medium font-roboto text-black">More about me</span>
                        <img src="data:image/png;base64," alt="arrow--v2" style={{maxHeight:"2rem"}}></img>

                </a>
            </div>

            {/* Image */}
            <img
                src={GalataTower}
                alt="About Image"
                className="w-full max-h-[50vh] lg:max-w-[40vw] lg:max-h-[80vh] object-contain mt-4 lg:mt-8"
                loading={directNavigation ? "eager" : "lazy"} // If directed from another page, do not lazy load.
            />
        </section>
    );
};

export default AboutSection;
